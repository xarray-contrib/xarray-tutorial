...
Title: _"Why does xarray need an index? (and when a custom one)"_. Intuition-first narrative:
...
- **Jupyter gotcha (bit us hard last time):** if the MCP warns _"collab socket disconnected /
  split-brain,"_ your edits may sit in a local buffer and never reach disk, and a stale
